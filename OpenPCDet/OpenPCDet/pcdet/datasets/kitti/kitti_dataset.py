...
class KittiDataset(DatasetTemplate):
    ...
    def get_lidar(self, idx):
        ...
        # assert lidar_file.exists()
        # data = np.load(str(lidar_file))

        try:
            if "bbox" in self.corruptions:
                data = MAP[self.corruptions](data,self.severity,idx)
            else:
                data = MAP[self.corruptions](data,self.severity)
        except:
            pass
    ...
    def get_image(self, idx):
        ...
        if self.corruptions == 'rain_sim':
            image_add_some_func = ImageAddRain(severity=self.severity, seed=seed)
            image = image_add_some_func(image, True,'./test.png')

        if self.corruptions == 'snow_sim':
            image_add_some_func = ImageAddSnow(severity=self.severity, seed=seed)
            image = image_add_some_func(image, True,'./test.png')

        if self.corruptions == 'fog_sim':
            image_add_some_func = ImageAddFog(severity=self.severity, seed=seed)
            image = image_add_some_func(image, True,'./test.png')

        if self.corruptions == 'gaussian_noise':
            image_add_some_func = ImageAddGaussianNoise(severity=self.severity, seed=seed)
            image = image_add_some_func(image, True,'./test.png')

        if self.corruptions == 'impulse_noise':
            image_add_some_func = ImageAddImpulseNoise(severity=self.severity, seed=seed)
            image = image_add_some_func(image, True,'./test.png')

        if self.corruptions == 'uniform_noise':
            image_add_some_func = ImageAddUniformNoise(severity=self.severity, seed=seed)
            image = image_add_some_func(image, True,'./test.png')

        if self.corruptions == 'gauss_sim':
            image_add_some_func = ImageAddGaussianNoise(severity=self.severity, seed=seed)
            image = image_add_some_func(image, True,'./test.png')
        
        
        if self.corruptions == 'impulse_sim':
            image_add_some_func = ImageAddImpulseNoise(severity=self.severity, seed=seed)
            image = image_add_some_func(image, True,'./test.png')

    
        if self.corruptions == 'uniform_sim':
            image_add_some_func = ImageAddUniformNoise(severity=self.severity)
            image = image_add_some_func(image, True,'./test.png')

        if  self.corruptions == 'motion_sim':
            severity=self.severity
            image_add_some_func = ImageMotionBlurFrontBack(severity)
            image = image_add_some_func(image, True,'./test.png')
    ...
    def get_calib(self, idx):
        ...
        if self.corruptions == 'spatial_alignment_noise':
            noise_pose = spatial_alignment_noise(calib.V2C,self.severity)
            calib.V2C = noise_pose
