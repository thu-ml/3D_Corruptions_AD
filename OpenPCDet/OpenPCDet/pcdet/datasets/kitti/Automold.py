...
def red(image, src='BGR'):
    ...
    if(is_list(image)):
        ...
        image_red = eval(
            'cv2.cvtColor(image,cv2.COLOR_'+src.upper()+'2RGB)[:,:,0]')
    return image_red
...
def shadow_process(image, no_of_shadows, x1, y1, x2, y2, shadow_dimension):
    ...
    vertices_list = generate_shadow_coordinates(
        imshape, no_of_shadows, (x1, y1, x2, y2), shadow_dimension)  # 3 getting list of shadow vertices
    for vertices in vertices_list:
        # adding all shadow polygons on empty mask, single 255 denotes only red channel
        cv2.fillPoly(mask, vertices, 255)
    # if red channel is hot, image's "Lightness" channel's brightness is lowered
    image_HLS[:, :, 1][mask[:, :, 0] ==
                       255] = image_HLS[:, :, 1][mask[:, :, 0] == 255]*0.5
    image_RGB = cv2.cvtColor(image_HLS, cv2.COLOR_HLS2RGB)  # Conversion to RGB
    return image_RGB
...
def add_shadow(image, no_of_shadows=1, rectangular_roi=(-1, -1, -1, -1), shadow_dimension=5):
    ...
    if(is_list(image)):
        ...
        output = shadow_process(image, no_of_shadows,
                                x1, y1, x2, y2, shadow_dimension)
        image_RGB = output
...
def rain_process(image, slant, drop_length, drop_color, drop_width, rain_drops):
    ...
    for rain_drop in rain_drops:
        cv2.line(image_t, (rain_drop[0], rain_drop[1]), (rain_drop[0] +
                 slant, rain_drop[1]+drop_length), drop_color, drop_width)
...
def add_rain(image, slant=-1, drop_length=20, drop_width=1, drop_color=(200, 200, 200), rain_type='None'):  # (200,200,200) a shade of gray
    ...
    if(is_list(image)):
        ...
        rain_drops, drop_length = generate_random_lines(
            imshape, slant, drop_length, rain_type)
        output = rain_process(image, slant_extreme,
                              drop_length, drop_color, drop_width, rain_drops)
        image_RGB = output
...
def add_fog(image, fog_coeff=-1):
    ...
    if(is_list(image)):
        ...
        for haze_points in haze_list:
            image = add_blur(
                image, haze_points[0], haze_points[1], hw, fog_coeff_t)
...
def flare_source_gaussian(image, point, radius, src_color, finetune_ratio, alpha=1):
    ...
    canvas = canvas_brighter_map * canvas + (1-canvas_brighter_map) * image
    return canvas
...
def add_sun_flare(image, flare_center=np.array([-1, -1]), angle=-1, no_of_flare_circles=8, src_radius=400, src_color=(255, 255, 255)):
    ...
    if(is_list(image)):
        ...
        if (flare_center == -1).all():
            ...
            flare_center_t = flare_center
        x, y = add_sun_flare_line(flare_center_t, angle_t, imshape)  # len(x)可能==0
        output = add_sun_process(
...
def apply_motion_blur(image, count):
    ...
    while(i <= imshape[1]):
        ...
        image_t[:, :imshape[1]-i,
                :] = cv2.filter2D(image_t[:, :imshape[1]-i, :], -1, kernel_motion_blur)
...
def add_manhole(image, center=-1, color=(120, 120, 120), height=1, width=1, type='closed'):
    verify_image(image)
    ...
    if(is_list(image)):
        ...
        for img in image_list:
            ...
            image_RGB.append(manhole_process(
                img, center_t, height_t, width_t, color))
    else:
        height_t = height
        width_t = width
        center_t = center
...
def augment_random(image, aug_types="", volume='expand'):
    ...
    aug_types_all = ["random_brightness", "add_shadow", "add_snow", "add_rain", "add_fog",
                     "add_gravel", "add_sun_flare", "add_speed", "add_autumn", "random_flip", "add_manhole"]
    if aug_types == "":
